...
instance Show State where
    show (St ip input memory output) = "IP#" ++ (Prelude.show ip) ++ " OUT=" ++ (Prelude.show output)
...
main = do
    putStrLn "Initializing super computer ..."
    contentParts <- getInputFromCommaList
    let program = parseProgram contentParts
    putStrLn "Solution 1:"
    print $ run (St 0 [1] program [])
    putStrLn "Solution 1:"
    print $ run (St 0 [5] program [])
